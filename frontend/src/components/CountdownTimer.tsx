...
export function CountdownTimer({ targetDate, initialSeconds, onEnd, label, className, showIcon = true }: CountdownTimerProps) {
    ...
    return (
        <div className={`flex items-center gap-2 font-mono font-bold transition-colors ${isUrgent ? "text-red-600 animate-pulse" : "text-gray-700"
            } ${className || ""}`}>
            {showIcon && (timeLeft < 3600 ? <Timer className="h-5 w-5" /> : <Clock className="h-5 w-5" />)}
            <div className="flex flex-col items-start leading-none">
                {label && <span className="text-[10px] uppercase tracking-wider text-gray-400 mb-0.5">{label}</span>}
